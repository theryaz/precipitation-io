...
class UltrasonicSensorDevice():
    ...
    def get_measurement(self) -> float:
        ...
        '''
        LOGGER.debug(f"[{self.name} UltrasonicSensorDevice]", f"getting measurement {HAS_REAL_HARDWARE}")
        if not HAS_REAL_HARDWARE:
            LOGGER.debug(f"[{self.name} UltrasonicSensorDevice]", "RPi env not detected, mocking Ultrasonic Sensor result")
            return randint(25, 95)
            
        # Send 20us pulse to trigger
        LOGGER.debug(f"[{self.name} UltrasonicSensorDevice]", f"Sending Pulse")
        GPIO.output(self._trig_pin, 1)
        sleep(0.00002)
        GPIO.output(self._trig_pin, 0)

        LOGGER.debug(f"[{self.name} UltrasonicSensorDevice]", f"Waiting for echo to go high")
        # Wait for echo to go high
        while GPIO.input(self._echo_pin) == 0:
            pass
        # Set the time, and wait for echo to go low
        LOGGER.debug(f"[{self.name} UltrasonicSensorDevice]", f"Waiting for echo to go low")
        start = time()
        ...
        LOGGER.debug(f"[{self.name} UltrasonicSensorDevice]", f"Returning result")
